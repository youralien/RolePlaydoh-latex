...
Simulation-based education methods, such as roleplay with peers~\cite{ay2023can} or Standardized Patients~\cite{kuehne2018standardized} and simulations with digital patients, can offer interactive practice opportunities that complement theoretical knowledge~\cite{kuhne2020standardized}.   However, each method has its limitations: practicing with peers is only viable if peers are experienced with roleplays; Standardized Patients are used mostly for evaluation rather than practice due to the cost and limited availability of trained, human actors; and most digital patients operate using tailored, dialogue trees~\cite{othlinghaus2020seriousroleplaying}, which limits the diversity of dialogue scenarios that can be practiced. Without appropriate practice opportunities, counselors may be underprepared or lack confidence for giving effective counseling support to real patients. Therefore, innovating beyond traditional methods by developing versatile, AI-powered simulation tools has the potential to revolutionize the training landscape for counselors, making practice opportunities more varied and readily available~\citep{yang2024social}.
...
However, these simulations, when evaluated by experienced counselors, often fail to resemble typical behaviors of real-patients such as using colloquial language and showing resistance to help~\cite{chen2023llmempowered}. 
Such AI patients are less useful roleplay partners for novice counselors if they make practice experiences artifically easy by not faithfully simulating the behaviors that are encountered in challenging, real-world counseling sessions. 
Thus, incorporating expert counselor feedback is essential for enhancing the realism of these simulations.
Popular methods for customizing models with human feedback involve collecting sparse binary or preference feedback for model re-training~\cite{christiano2017deep, rafailov2024direct} but can be tedious and time-consuming especially for domain-experts.
In settings like mental health where expert feedback is costly, there is a need for more efficient methods that allow expert counselors to provide more dense, free-form feedback to refine AI simulations to better mimic realistic patient behavior. 

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/rpdteaser-clean.png}
    \caption{Roleplay-doh empowers an expert counselor to create a customized AI patient intended for other novice counselors to use as a practice partner. While interacting with the AI patient, the expert counselor can provide qualitative feedback which is converted by an LLM call into a principle, or a custom rule defining desired roleplay behavior, which is then appended to the AI Patient's Constitution that governs the patient's roleplay. The AI patient references the latest AI Patient Constitution to generate responses that follow the expert-defined principles.}
    \label{fig:rpdteaser}
\end{figure*}

In this paper, we study how to design a human-LLM collaboration pipeline that empowers a counseling expert to create and customize an LLM patient that could eventually be used by a novice counselor for realistic practice. 
% Explain design goals for roleplay? Then we can say why a recent paradigm would be applicable?
Since counselors may not know how to prompt LLMs to simulate desired personas and behaviors~\cite{whyjohnnycantprompt}, the pipeline should give them the ease and flexibility of giving feedback in their own words while still effectively steering the simulation. Second, since counseling experts may have multiple behaviors to customize, its useful to be able to rapidly test and iterate on different aspects of their simulation, which rules out methods requiring model re-training. Towards these design goals,  
our pipeline adopts a recent paradigm for user-driven chatbot design~\cite{petridis2023constitutionmaker} that allows users to customize LLM chatbots by interactively critiquing model responses via qualitative feedback. Through get converted into constitutional \textit{principles}~\cite{bai2022constitutional}. 
...
(Fig~\ref{fig:rpdteaser}). With Roleplay-doh, a counselor creates an AI patient scenario by describing a past challenging case and interactively customizes the roleplay behaviors by providing free-form feedback on dialogue responses while interacting. An LLM module helps to convert expert feedback into a well-specified principle, which can be thought of as a rule that the AI patient should follow, \textit{i.e., "Respond to encouraging words with hesitation, doubting their significance".} Then, the LLM-simulation generates the patient's future responses according to a constitution consisting of these expert-defined principles. 

However, our preliminary analyses show that directly prompting GPT-4 to simulate the AI patient's scenario and expert-defined principles results in poor responses in 20\% of cases, primarily due to errors in satisfying multipart principles or those that are only applicable in specific contexts e.g., only when the therapist provides encouraging words.
...
In the mental health area, researchers have begun using human-centered design approaches to involve therapy experts when prompting LLMs for simulation~\cite{chen2023llmempowered, lin2024imbue}. In this way, qualitative feedback is gathered during testing sessions with experts, which informs how a researcher engineers the LLM prompts. However, requiring a researcher-in-the-loop to refine prompts hinders the speed of iterative design, which we aim to eliminate through our work.  % upon a customized LLM-simulation. 
...
tool for mental health counselors to construct their own AI patients, by conducting formative studies with counselors to understand what AI patients they want to create, and their needs for effective human-LLM collaboration when constructing an AI patient. % (Section~\ref{sec:formative-tests}).  
...
Counselors start by writing a description of a patient scenario, or users can equivalently refer to or select from a list of example scenarios. Then, they can begin chatting with a LLM-powered AI patient whose responses are conditioned on the patient description and conversation history. To support rapid prototyping of the AI patient's behaviors, the tool adopts a recent paradigm for user-driven chatbot design~\cite{petridis2023constitutionmaker}, in which users can provide qualitative feedback on the AI patient chatbot's response which gets converted by an LLM call into a constitution made of {\em principles} that govern desired chatbot behavior (Fig~\ref{fig:rpdteaser}).
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/principle-adherence-clean.png}
...
\begin{table}[t]
    ...
    \begin{tabular}{|l|c|l|} \hline 
          \textbf{Measure}&\textbf{Scenario Only}&\textbf{+ Principles}\\ \hline 
          Authenticity&5.25&+0.79 ** \\ \hline 
          Stayed in Role&6.29&+0.08\\ \hline 
          Resembled Past Case&4.91&+0.71 * \\ \hline 
          Mirrored Hard Aspects &4.62&+0.91 * \\ \hline 
          Ready as Training Partner&5.25&+0.66 *  \\ \hline 
          Recommend to Novices&5.87&+0.5 * \\ \hline
    \end{tabular}
    }
    \caption{\small{Creators (N=24) rated their own \textit{Scenario-Only} vs \textit{Scenario+Expert Principles} AI patients along six measures using a 7-point Likert-scale. After refining the AI patient simulation with Expert Principles, creators rate the patient significantly higher on all measures except for \textit{stayed in role}, for which both AI patients score highly. Results remain significant after using the ~\citet{benjamini1995controlling} method to correct for multiple comparisons ($Q=0.05$). (***:$p<.001$, **:$p<0.01$, *:$p<0.05$., .:$p<0.1$)}}
...
\textit{\textbf{Tool User Experience:}} Counselors found the tool helpful for writing principles that \textbf{effectively guided} the AI patient to recreate their past case ($\mu=6.04$, $\sigma=1.08$). With the tool, it was \textbf{easy} to convert their thoughts and feedback on the AI patient's behavior into principles ($\mu=6.08$, $\sigma=1.13$). Participants also felt they could \textbf{efficiently} write principles ($\mu=6.17$, $\sigma=1.3$), without requiring much \textbf{mental demand} ($\mu=3.29$, $\sigma=1.68$). 
...
The final version of Roleplay-doh (Fig~\ref{fig:rpd}) generates responses in the LLM simulation using a self-critique-revise module. In addition to this core improvement, we made several minor improvements to improve the usability and user experience of the tool. 
